...
class SiameseReId():
    ...
    def similarity(self, img_tid, det_fv):
        self.model.eval()
        img_tid_fv, det_fv = self.preprocess(img_tid).to(self.device).unsqueeze(0), det_fv.to(self.device)
        return self.model(img_tid_fv, det_fv)
