...
model = YOLO("yolov8m-seg.pt") # COCO128 classes https://github.com/ultralytics/yolov5/blob/master/data/coco128.yaml
...
def exec_re_id(_video_in,_video_out,total_frame=-1):

    file_path = _video_in
    cap = cv2.VideoCapture(file_path)
    video_out = cv2.VideoWriter(_video_out, cv2.VideoWriter_fourcc(*'DIVX'), int(cap.get(cv2.CAP_PROP_FPS)), (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))))
    ...
    vid_w, vid_h, vid_length = int(cap.get(3)), int(cap.get(4)), int(cap.get(7))
    ...
    bar = progressbar.ProgressBar(max_value=total_frame, widgets=widgets).start()
    ...
    while frame_n < total_frame:
        ...
        for det_i in range(frames_window):
            det = buffer_detections[det_i]
            det["box_id"]= det["box_id"].apply(lambda x: -1)
            det["track_id"] = det["box_id"]
            buffer_detections[det_i] = det.drop(['fv'],axis=1).copy()

        # swap and progress with ciruclar buffer
        for f_swap in range(frames_window//2):
            buffer_frames[f_swap] = buffer_frames[f_swap+frames_window//2].copy()
            buffer_detections[f_swap] = buffer_detections[f_swap+frames_window//2].copy()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Person RE-ID, produce a video with labaeled persons')
    ...
    exec_re_id(args.video_in,args.video_out,int(args.n_frames))