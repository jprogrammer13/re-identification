...
def crop_segmentation(_mask, _box, _image):
    ...
    w, h, _ = img.shape
    mask = (cv2.resize(_mask, (h, w)) > 0).astype("uint8")
...
def exec_re_id(_video_in, _video_out, _frames_window=10, total_frame=-1):
    ...
    # use total number of frame if -1 is passed
    if total_frame == -1:
                            ...
                            if int(row['track_id']) == -1:
                                if row['box_id'] not in added:
                                    ...
                                    color = np.random.randint(
                                        0, 255, 3).tolist()
                                    fv = siamese_net.fv_encoding(Image.fromarray(crop_segmentation(
                                        np.array(row['mask']), np.array(row['box']), buffer_frames[i]))).cpu().numpy()
                                    track_id_df = pd.concat([track_id_df, pd.DataFrame(
                                        {"fv": [fv], 'color':[color]})], ignore_index=True)
